...
public class MybatisGeneratorUtil {
	...
	// generatorConfig模板路径
	private static String generatorConfig_vm_pinet = "/template/generatorConfig_pinet.vm";
	private static String generatorConfig_vm = "/template/generatorConfig.vm";
	// Service模板路径
	private static String service_vm_pinet = "/template/Service_pinet.vm";
	private static String service_vm = "/template/Service.vm";
	// ServiceMock模板路径
	private static String serviceMock_vm = "/template/ServiceMock.vm";
	// ServiceImpl模板路径
	private static String serviceImpl_vm_pinet = "/template/ServiceImpl_pinet.vm";
	 ...
	 * @param package_name  包名
	 */
	public static void generator_pinet(
			String jdbc_driver,
			String jdbc_url,
			String jdbc_username,
			String jdbc_password,
		...
		generatorConfig_vm = MybatisGeneratorUtil.class.getResource(generatorConfig_vm_pinet).getPath().replaceFirst("/", "");
		service_vm = MybatisGeneratorUtil.class.getResource(service_vm_pinet).getPath().replaceFirst("/", "");
		serviceMock_vm = MybatisGeneratorUtil.class.getResource(serviceMock_vm).getPath().replaceFirst("/", "");
		serviceImpl_vm = MybatisGeneratorUtil.class.getResource(serviceImpl_vm_pinet).getPath().replaceFirst("/", "");

		String targetProject = module ;
		String basePath = MybatisGeneratorUtil.class.getResource("/").getPath().replace("/target/classes/", "").replace(targetProject, "").replaceFirst("/", "");
		String projectPath = basePath.replace("zheng-dao", "");
		String generatorConfig_xml = MybatisGeneratorUtil.class.getResource("/").getPath().replace("/target/classes/", "") + "/src/main/resources/generatorConfig.xml";
		...
		try {
			...
			String targetProject_sqlMap = projectPath  + "zheng-dao" ;

			String m = package_name.substring(package_name.lastIndexOf('.') + 1);

			System.out.println("targetProject_sqlMap:" + targetProject_sqlMap);
			...
			context.put("targetProject", targetProject);
			context.put("targetProject_sqlMap", targetProject_sqlMap);
			context.put("targetProject_sqlPackage", m);
			context.put("generator_jdbc_password", AESUtil.AESDecode(jdbc_password));
			context.put("last_insert_id_tables", last_insert_id_tables);
			VelocityUtil.generate(generatorConfig_vm, generatorConfig_xml, context);
			// 删除旧代码
//			deleteDir(new File(targetProject + "/src/main/java/" + package_name.replaceAll("\\.", "/") + "/model"));
			...
			deleteDir(new File(targetProject_sqlMap + "/src/main/java/" + package_name.replaceAll("\\.", "/") + "/mapper"));
		} catch (Exception e) {
			e.printStackTrace();
		}
		System.out.println("========== 结束生成generatorConfig.xml文件 ==========");
		...
		System.out.println("========== 开始生成Service ==========");
		String ctime = new SimpleDateFormat("yyyy/M/d").format(new Date());
		String servicePath = projectPath + module + "/src/main/java/" + package_name.replaceAll("\\.", "/") + "/service";
		String serviceImplPath = projectPath + module + "/src/main/java/" + package_name.replaceAll("\\.", "/") + "/service/impl";
