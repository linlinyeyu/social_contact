...
//    }
    ...
    public static int[] split2IntoIntArray(String s, String seperator) {
        if (isEmpty(s)) return new int[0];
    ...
    public static List<Short> split2IntoShort(String s, String seperator) {
        if (isEmpty(s)) return Lists.newArrayList();
        List<Short> ids = Lists.newArrayList();
        for (String temp : Splitter.on(seperator).split(s)) {
            try {
                ids.add(Short.parseShort(temp));
            } catch (Exception e) {
                e.printStackTrace();
            }
        }
    ...
    public static short[] split2IntoShortArray(String s, String seperator) {
        if (isEmpty(s)) return new short[0];

        String[] ss = s.split(seperator);

        short[] ids = new short[ss.length];

        int i = 0;
        for (String temp : ss) {
            try {
                ids[i++] = Short.parseShort(temp);
            } catch (Exception e) {
                e.printStackTrace();
            }
        }
