...
public class RedisUtil {
	...
	public synchronized static <T> T getObject(String key, Class<T> cls){
	...
	public synchronized static <T> void rpushObject(String key, T t){
		if(key == null || value == null){
			return;
		}
		String v = JSONObject.toJSONString(t);
		rpushString(key, v);
	}

	public synchronized static void rpushString(String key, String value){
		Jedis jedis = getJedis();
		if (null == jedis) {
			return;
		}
	...
	public synchronized static <T> T lpop(String key, Class<T> cls){
		...
		T value = JSONObject.parseObject(v, cls);

		return value;
	}

	public synchronized static <T> List<T> lrange(String key, long start, long end, Class<T> cls){
	 ...
	 * @return value
	 */
	public synchronized static Long incr(String key, long value, int aliveTime) {
		Jedis jedis = getJedis();
		if (null == jedis) {
			return null;
		}
		...
		long result = value;
		if(v == null){
			jedis.setex(key, aliveTime, "" + value);
		}else{
